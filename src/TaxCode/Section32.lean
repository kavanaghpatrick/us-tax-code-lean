...
This project request had uuid: f2dde993-ddb7-4383-9902-7f7fab189977
...
theorem example_2_value : calculate_credit example_profile_2 example_tax_year = 737 := by
  ...
  -- Calculate the phaseout threshold.
  simp +decide [example_profile_2]
...
theorem excess_investment_income_ineligible (tp : TaxpayerProfile) (ty : TaxYear) :
  ...
  -- Since the investment income exceeds the limit, the condition is false, making the entire implication true.
  intros h_filing_status h_tin h_spouse_tin h_nonresident h_section_911 h_qualifying_child h_investment_income
  exact absurd h_investment_income (not_le_of_gt h_excess)
...
theorem non_mfs_may_qualify (tp : TaxpayerProfile) (ty : TaxYear) :
  ...
  negate_state;
  -- Proof starts here:
  -- Let's choose any tax year, say 2023.
  use example_profile_2, example_tax_year;
  -- Let's unfold the definition of `example_profile_2` to verify its filing status.
  simp +decide [example_profile_2]

-/
-- §32(d): Non-MFS filing statuses can qualify (existence proof)
-- Shows that Single filers can be eligible for EITC
theorem single_filer_may_qualify :
  ∃ (tp : TaxpayerProfile) (ty : TaxYear),
    tp.filing_status = FilingStatus.Single ∧
    is_eligible_individual tp ty = true := by
  use example_profile_1
  use example_tax_year
  constructor
  · rfl
  · unfold is_eligible_individual
    simp [example_profile_1, example_tax_year]
    decide
